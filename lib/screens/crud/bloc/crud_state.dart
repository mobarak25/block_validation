...
class SheetInput extends CrudState {
  final String newTaskStat;
  const SheetInput({this.newTaskStat = ''});

  SheetInput copy({String? newTaskState}) {
    copyWith(
      newTaskState: newTaskState ?? this.newTaskState,
    );
    return SheetInput(
      newTaskStat: newTaskState ?? this.newTaskState,
    );
  }

  @override
  List<Object> get props => [newTaskState];
}
